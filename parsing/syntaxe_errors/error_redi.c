...
/*   Updated: 2025/07/26 11:27:34 by yabounna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Corrigé : accepte redirection même sans target immédiat (ex: here-doc)
static int check_redir_target(char *str, int *i)
{
    char quote;

    if (!str[*i]) // rien après la redirection
        return printf_error();
    ...
    {
        while (str[*i] && !skip_space(str[*i]) && str[*i] != '|' && str[*i] != '<' && str[*i] != '>')
            (*i)++;
    }
